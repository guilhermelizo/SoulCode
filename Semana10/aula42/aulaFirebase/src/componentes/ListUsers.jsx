...
export default function ListUsers({navigation}){
    ...
    // Esta conexão será praticamente padrão, trocando apenas para a conexão desejada no lugar de 'users'
    const pegaDados = async () =>{
        const users = firebase.db.collection('users');
        const querySnapShot = await users.get();
        const dados = querySnapShot.docs;

        //listUsers inicia um array vazio e com o .push receberá um objeto com os dados do documento
        const listUsers = [];
        /**
         * listUsers é um arrau que vai criar um {objeto} com varios {objetos}
    ...
    return(
        <View style={styles.container}>
            ...
            <Button    // Botão que leva para a página de adicionar usuários (AddUsers)
